...
| `MARSCLAW_ALLOW_SHELL` | `1` to expose the Bash tool to the agent (default `0`) — see [security.md](security.md) |
...
Four flags govern what the agent can do beyond reading approved files. All are **off by default**; turning each one on reopens a specific attack surface that [security.md](security.md) walks through in detail.

...
[NanoClaw](https://github.com/qwibitai/nanoclaw) is the upstream multi-tenant agent platform. marsClaw is a personal-scale rewrite that runs an off-the-shelf agent SDK (the Claude Agent SDK or the Gemini CLI core) — **in-process by default**, with an opt-in **container runtime** for isolation — instead of orchestrating a container *per session*.

> Earlier versions of this doc described marsClaw as a thin wrapper that shelled out to a CLI once per message, with "no container" as a flat fact. Both are now stale: marsClaw embeds the agent SDK as a library, keeps one long-lived session per chat thread, and has grown a Google Workspace tool suite, a service installer, backups, budgeting, conversation archiving, and an **optional container runtime** (`runtime: container`, see [container-runtime.md](container-runtime.md)). The remaining structural difference from NanoClaw is *per-session* vs *single shared* containerization, not container-vs-no-container. The sections below reflect the current code.
...
| Isolation | default: in-process as host user (capability-removal + `allowed_paths`); optional: **one shared isolated container** for the single owner | per-session Docker / Apple Container, one per tenant |
| Agent runtime | Claude Agent SDK **or** Gemini CLI core, in-process; one long-lived session per thread — optionally inside a shared container | Claude Agent SDK in-container; OpenCode via providers branch |
...
| | Pros | Cons |
|---|---|---|
| **marsClaw** — single Bun process; the agent SDK runs in-process with one long-lived session per thread (LRU-capped at `max_sessions`, idle/age-swept). Optionally the SDK loop runs in one shared, lazy-started container with the host as broker | Trivial to debug; one process to inspect; no IPC, no Docker in the default mode; cold-start (~10s SDK/googleapis import) paid once per chat, not per message; sessions resume from the on-disk transcript after a recycle or crash; container mode adds host isolation without going multi-tenant | In-process: one crash kills everything, agent shares the host fs (mitigated, not eliminated, by `allowed_paths`), no per-conversation isolation. Container mode adds host isolation but it's still **one shared box for all threads** — not per-conversation |
...
Note the runtime mechanism is no longer the dividing line — both sides now embed the Claude Agent SDK, and both can run it in a container. The real difference is *granularity*: NanoClaw runs **one container per session** (true multi-tenant isolation); marsClaw's optional container is **one shared box for its single owner** (host isolation, not tenant isolation).

## Isolation & security

| | Pros | Cons |
|---|---|---|
| **marsClaw** | **In-process action broker:** the MCP server is the sole egress path and the only process that holds Google creds (Anthropic key withheld from it by env passthrough). **Default-deny capability flags:** `allow_shell`, `allow_web`, `allow_mutating_tools` are all off out-of-the-box — fresh install has no third-party exfil channel. **Sensitive-path guard** blocks `.env`, `data/secrets`, `data/config.json`, `data/whatsapp-auth`, `data/marsclaw.db`, `~/.claude.json`, `~/.gemini` from FS tools *regardless* of `allowed_paths`; `Grep`/`Glob` recursion gate refuses search roots that straddle them. **Web allow-list + researcher subagent:** web reads delegated to a `tools:['WebFetch']`-only subagent in an empty-room context, gated to approved hosts. **Per-channel sender allow-lists** (Telegram chat ids, Slack user ids, WhatsApp JIDs). **Append-only audit log** of every tool decision at `logs/audit.log`. Plus the existing rate-limits, budget cap, per-thread serialization, backups. See [security.md](security.md). **Optional container runtime:** flip `runtime: container` and the SDK loop moves into an isolated box — `.env`/DB/creds never mounted, the real Anthropic credential held by a host-side proxy, web egress through an SSRF gateway — so a malicious dependency or full shell no longer reaches host secrets (see [container-runtime.md](container-runtime.md)). | In the **default in-process** mode: enforcement is process-boundary + policy, not kernel; a malicious dependency or a re-enabled shell bypasses the in-process gates; channel tokens sit in `.env` on disk; the audit log is local-only (no remote sink). **Container mode** closes the host-secret class but is still one shared box (no per-conversation isolation), trades the URL allow-list for raw SSRF-filtered web egress (public-web exfil not closed), and has open hardening items (e.g. egress gateway binds `0.0.0.0`) |
...
What's changed is that the *runtime* distinction has mostly collapsed: marsClaw used to shell out to a CLI per message, but it now embeds the same Claude Agent SDK NanoClaw uses, keeps warm sessions, has grown real operational machinery (service install, backups, budgeting, archiving, observability), and can even run its agent in a container. The remaining difference is granularity, not mechanism — NanoClaw runs **each session** in its own container with users, roles, and approval flows; marsClaw runs everything for **one owner**, either in-process (sandboxed to an `allowed_paths` allowlist) or in **one shared container** that isolates the host but not individual conversations.
